...
export default function HomeView() {
	return (
		<Md>
			...
			<HH>제품 소개</HH>
			<PP2>
				판다교육에서 현재 사용하고 있는 프로그램입니다. 선생님들과 조교들의
				편의를 위해, 그리고 학원의 체계적인 시스템 구축을 위해 사용하고
				있습니다.
			</PP2>
			<HH>QR 코드 생성</HH>
			<PP1>QR 코드를 생성합니다.</PP1>
			<PP2>
				기존의 QR 코드를 만들어주는 홈페이지들은 한 번에 하나씩밖에 얻을 수
				없었습니다. 이 프로그램을 이용하면 한번에 여러 QR 코드를 다운로드할 수
				있습니다. 대량의 QR 코드를 생성하고 싶을 때 유용합니다.
			</PP2>
			<PP2>
				링크 입력 : 입력 칸에 동영상 링크 또는 홈페이지 링크를 입력하세요.
				유튜브 플레이리스트의 경우 플레이리스트 갯수만큼 생성됩니다.
			</PP2>
			<PP2>변환 : Convert 버튼을 누르면 링크가 QR 코드로 변환됩니다.</PP2>
			<PP2>
				모두 다운로드 : 생성된 QR 코드를 zip 파일로 모두 다운로드합니다. 이때
				파일명은 모두 'QR 코드 모음집' 이므로 여러 개의 zip 파일을 다운로드할
				경우 파일 이름을 바꿔주세요.
			</PP2>
			<PP2>모두 삭제하기 : 모든 QR 코드가 삭제됩니다.</PP2>
			<PP2>
				개별 다운로드 : QR 코드를 변환하면 화면상에 각각의 QR 코드가 나옵니다.
				QR 코드를 눌러 다운로드하거나 X 버튼을 눌러 개별 삭제할 수 있습니다.
			</PP2>
		</Md>
	);
}
